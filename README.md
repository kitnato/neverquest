...
The looted resources can only be collected once the [wilderness](#wilderness) is cleared of monsters.
...
Engage and fight monsters. Every wilderness level has a certain amount of monsters of a certain power determined by the level and current progress. The name of the wilderness level is randomly generated by [LOCRA](#locra).

The level is considered completed when the progress is at its maximum after having killed all the monsters. At this point, all [loot](#looting) can be collected and the character can progress to the [caravan](#caravan).
...
Grants a Well Fed [buff](#buffs) for the next wilderness level (+10% [mastery](#mastery) gain).
...
Allows the character to return to any previous wilderness level to hunt more monsters.

##### Hearthstone

Allows the character to return to the caravan regardless of if the current [wilderness'](#wilderness) monsters are cleared out or not. Can only be used if not currently attacking (so that the inventory can be opened to use the hearthstone).
