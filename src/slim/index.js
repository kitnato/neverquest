...
export default class SLIM {
  static get categories() {
    return {
      order: ["stranger", "location", "item", "monster"],
      stranger: {
  ...
  static generate(categoryName) {
    ...
    category.order.forEach((partID) => {
      if (Math.random() <= category[partID].probability) {
        collection = SLIMDB.filter(
          (entry) =>
            Object.keys(entry).includes(categoryName) &&
            entry[categoryName].includes(partID)
        );
    ...
    result = parts.map(
      (subPart) => subPart.name.charAt(0).toUpperCase() + subPart.name.slice(1)
    );
    ...
    partIDs.forEach((partID, index) => {
      ...
      isType = SLIM.categories.order.some(
        (subCategoryName) =>
          part[subCategoryName] && part[subCategoryName].includes("type")
      );
      isTheOne =
        category[partID].theOne &&
        Math.random() <= category[partID].theOne.probability;
      // isPossessive =
      //   category[partID].possessive && Math.random() <= category[partID].possessive.probability;
      hasTitle =
        category[partID].title &&
        Math.random() <= category[partID].title.probability;
